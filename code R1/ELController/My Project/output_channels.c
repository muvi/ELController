...
/*
SineWaveSample tc3_next_sample = SINE_WAVE_DEFAULT_SAMPLE;
SineWaveSample tc4_next_sample = SINE_WAVE_DEFAULT_SAMPLE;
...
//void TC3_Handler() {
	//channel 4
/*
	hri_tc_clear_interrupt_OVF_bit(TC3);
	SineWaveSample next_value = sine_wave_next(&(output_channels_wave_form[3]));
	hri_tccount8_write_CC_reg_no_lock(TC3, 0, next_value.pos);
	hri_tccount8_write_CC_reg_no_lock(TC3, 1, next_value.neg);
	*/
	/*
	if (hri_tc_get_interrupt_OVF_bit(TC3)) {
		hri_tc_clear_interrupt_OVF_bit(TC3);
		if (tc3_next_sample.incrementing) {
			hri_tccount8_write_CC_reg_no_lock(TC3, 0, tc3_next_sample.pos);
		} else {
			hri_tccount8_write_CC_reg_no_lock(TC3, 1, tc3_next_sample.neg);
		}
		tc3_next_sample = sine_wave_next(&(output_channels_wave_form[3]));
	} else if (hri_tc_get_interrupt_MC0_bit(TC3)) {
		hri_tc_clear_interrupt_MC0_bit(TC3);
		if (!tc3_next_sample.incrementing) {
			hri_tccount8_write_CC_reg_no_lock(TC3, 0, tc3_next_sample.pos);
		}
	} else if (hri_tc_get_interrupt_MC1_bit(TC3)) {
		hri_tc_clear_interrupt_MC1_bit(TC3);
		if (tc3_next_sample.incrementing) {
			hri_tccount8_write_CC_reg_no_lock(TC3, 1, tc3_next_sample.neg);
		}
	}

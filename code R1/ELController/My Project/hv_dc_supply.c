...
static inline void hv_dc_supply_sense(const uint16_t voltage) {
	...
	switch (hv_dc_supply_state) {
		case HV_DC_SUPPLY_STATE_INITIALIZE: {
			gpio_set_pin_level(GPIO1, false);
...
void hv_dc_supply_init_timer() {
	...
	//for debugging only---
	gpio_set_pin_level(GPIO11, true);
	gpio_set_pin_direction(GPIO11, GPIO_DIRECTION_OUT);
	gpio_set_pin_drive_strength(GPIO11);
	gpio_set_pin_function(GPIO11, PINMUX_PB30F_TCC1_WO2);
